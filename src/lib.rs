//! Rust wrapper for [HWI](https://github.com/bitcoin-core/HWI/).
//!
//! # Example
//! ```
//! use hwi::{interface, types};
//! use hwi::error::Error;
//! use bitcoin::util::bip32::{ChildNumber, DerivationPath};
//! use std::str::FromStr;
//!
//! fn main() -> Result<(), Error> {
//!     let devices = interface::HWIDevice::enumerate()?;
//!     let device = devices.first().unwrap();
//!     let derivation_path = DerivationPath::from_str("m/44h/1h/0h/0/0").unwrap();
//!     let hwi_address = device.display_address_with_path(&derivation_path, types::HWIAddressType::Legacy, types::HWIChain::Test)?;
//!     println!("{}", hwi_address.address);
//!     Ok(())
//! }
//! ```

#[cfg(test)]
#[macro_use]
extern crate serial_test;

pub use interface::HWIDevice;

pub mod error;
pub mod interface;
pub mod types;

#[cfg(test)]
mod tests {
    use crate::interface::HWILib;
    use crate::types;
    use crate::HWIDevice;
    use std::str::FromStr;

    use bitcoin::util::bip32::DerivationPath;
    use pyo3::PyObject;

    #[test]
    #[serial]
    fn test_enumerate() {
        let lib = HWILib::initialize().unwrap();
        let devices = HWIDevice::enumerate(&lib).unwrap();
        assert!(devices.len() > 0);
    }

    fn get_first_device(lib: &HWILib) -> (HWIDevice, PyObject) {
        let hwidev = HWIDevice::enumerate(lib)
            .unwrap()
            .first()
            .expect("No devices")
            .clone();
        let dev = hwidev
            .find_device(lib, true, types::HWIChain::Test)
            .unwrap();
        (hwidev, dev)
    }

    #[test]
    #[serial]
    fn test_get_master_xpub() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        device
            .get_master_xpub(&client, types::HWIAddressType::Wit, &lib, 0)
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_get_xpub() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/44'/1'/0'/0/0").unwrap();
        device
            .get_xpub(&client, &derivation_path, &lib, false)
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_get_xpub_with_expert_flag_set() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/44'/1'/0'/0/0").unwrap();

        device
            .get_xpub(&client, &derivation_path, &lib, false)
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_sign_message() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/44'/1'/0'/0/0").unwrap();
        device
            .sign_message(
                &client,
                "I love magical bitcoin wallet",
                &derivation_path,
                &lib,
            )
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_get_descriptors() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let account = Some(10);
        let descriptor = device.get_descriptors(&client, account, &lib).unwrap();
        assert!(descriptor.internal.len() > 0);
        assert!(descriptor.receive.len() > 0);
    }

    #[test]
    #[serial]
    fn test_display_address_with_desc() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        println!("{}", client);
        let descriptor = device.get_descriptors(&client, None, &lib).unwrap();
        let descriptor = descriptor.receive.first().unwrap();
        // Seems like hwi doesn't support descriptors checksums
        let descriptor = &descriptor.split("#").collect::<Vec<_>>()[0].to_string();
        let descriptor = &descriptor.replace("*", "1"); // e.g. /0/* -> /0/1
        device
            .display_address_with_desc(&client, &descriptor, &lib)
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_display_address_with_path_legacy() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/44'/1'/0'/0/0").unwrap();
        device
            .display_address_with_path(
                &client,
                &derivation_path,
                types::HWIAddressType::Legacy,
                &lib,
            )
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_display_address_with_path_nested_segwit() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/49'/1'/0'/0/0").unwrap();

        device
            .display_address_with_path(
                &client,
                &derivation_path,
                types::HWIAddressType::Sh_Wit,
                &lib,
            )
            .unwrap();
    }

    #[test]
    #[serial]
    fn test_display_address_with_path_native_segwit() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let derivation_path = DerivationPath::from_str("m/84'/1'/0'/0/0").unwrap();

        device
            .display_address_with_path(&client, &derivation_path, types::HWIAddressType::Wit, &lib)
            .unwrap();
    }

    // TODO: HWI 2.0.2 doesn't support displayaddress with taproot
    // #[test]
    // #[serial]
    // fn test_display_address_with_path_taproot() {}

    // TODO: Create PSBT with scratch using given Hardware Wallet
    // #[test]
    // #[serial]
    // fn test_sign_tx() {}

    #[test]
    #[serial]
    fn test_get_keypool() {
        let lib = HWILib::initialize().unwrap();
        let (device, client) = get_first_device(&lib);
        let keypool = true;
        let internal = false;
        let address_type = types::HWIAddressType::Legacy;
        let account = Some(8);
        let derivation_path = DerivationPath::from_str("m/44'/1'/0'").unwrap();
        let start = 1;
        let end = 5;
        device
            .get_keypool(
                &client,
                keypool,
                internal,
                address_type,
                false,
                account,
                Some(&derivation_path),
                start,
                end,
                &lib,
            )
            .unwrap();

        let keypool = true;
        let internal = true;
        let address_type = types::HWIAddressType::Wit;
        let account = None;
        let start = 1;
        let end = 8;
        device
            .get_keypool(
                &client,
                keypool,
                internal,
                address_type,
                false,
                account,
                None,
                start,
                end,
                &lib,
            )
            .unwrap();

        let keypool = false;
        let internal = true;
        let address_type = types::HWIAddressType::Sh_Wit;
        let account = Some(1);
        let start = 0;
        let end = 10;
        device
            .get_keypool(
                &client,
                keypool,
                internal,
                address_type,
                false,
                account,
                Some(&derivation_path),
                start,
                end,
                &lib,
            )
            .unwrap();
    }
}
